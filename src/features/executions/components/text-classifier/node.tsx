...
export const TextClassifierNode = memo((props: NodeProps<T>) => {
  ...
  const handleSubmit = (v: TextClassifierFormValues) =>
    setNodes((ns) =>
      ns.map((n) =>
  ...
  return (
    <>
      <TextClassifierDialog
        open={open}
        onOpenChange={setOpen}
        onSubmit={handleSubmit}
        defaultValues={props.data as Partial<TextClassifierFormValues>}
      />
      <BaseExecutionNode
        {...props}
        id={props.id}
        ...
        description={
          props.data.categories
            ? String(props.data.categories).slice(0, 40)
            : "Not configured"
        }
